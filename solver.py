...
class Solver():
    ...
    def setupParams(self):
        ...
        # convert to m/s
        self.gt = np.array(gt[:self.len_gt]) * 1.6 * 5/16
    ...
    def processFrame(self, frame):
        ...
        curr_pts, _st, _err = cv2.calcOpticalFlowPyrLK(frame, self.prev_gray, self.prev_pts, None, **self.lk_params)
    ...
    def run(self):
        ...
        hf_factor = np.linalg.lstsq(preds.reshape(-1,1),
                                    gt_train, rcond=-1)[0][0]
    ...
    def visualize(self, frame, mask_vis, prev_key_pts):
    ...
    def test(self):
        ...
        while self.test_vid.isOpened():
            ...
            if self.prev_gray is None:
                ...
                # Get median of predicted V/hf values
                preds = self.processFrame(frame_gray)
                self.temp_preds[frame_idx] = np.median(preds) if len(preds) else 0
